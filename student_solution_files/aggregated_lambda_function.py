...
def email_response(name, critical_phrase_list, list_of_extracted_phrases, AWS_Comprehend_Sentiment_Dump):
    ...
    SENDER_NAME = 'Ndivhuho Judith Magwede'
...
def lambda_handler(event, context):
    ...
    rid = random.randint(101, 1000000000)
    ...
    key_phrases = comprehend.detect_key_phrases(Text=enquiry_text, LanguageCode='en')['KeyPhrases']
   
    phrase = []
    for i in range(0, len(key_phrases)-1):
        phrase = np.append(phrase, key_phrases[i]['Text'])

    critical_phrase_list = ['github', 'git', 'Git', 'GitHub', 'projects', 'portfolio', 'Portfolio', 'C.V', 'resume', 'Curriculum Vitae', 'Resume', 'CV', 'C.V.']
